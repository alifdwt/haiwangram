...
func createRandomPhoto(t *testing.T) *models.Photo {
	...
	photo, err := testRepository.Photo.CreatePhoto(userId, *photoArg)
	require.NoError(t, err)
	...
	// Error not found
	_, err = testRepository.Photo.CreatePhoto(0, *photoArg)
	require.Error(t, err)
...
func TestGetPhotoById(t *testing.T) {
	...
	res, err := testRepository.Photo.GetPhotoById(photo.ID)
	require.NoError(t, err)
	...
	// Error not found
	_, err = testRepository.Photo.GetPhotoById(0)
	require.Error(t, err)
}
...
func TestUpdatePhoto(t *testing.T) {
	...
	photo2, err := testRepository.Photo.UpdatePhoto(photo1.UserID, photo1.ID, *photoArg)
	require.NoError(t, err)
	...
	// Error not found
	_, err = testRepository.Photo.UpdatePhoto(0, 0, *photoArg)
	require.Error(t, err)
}
...
func TestDeletePhoto(t *testing.T) {
	...
	_, err := testRepository.Photo.DeletePhoto(photo1.ID)
	require.NoError(t, err)

	_, err = testRepository.Photo.GetPhotoById(photo1.ID)
	require.Error(t, err)

	// Error not found
	_, err = testRepository.Photo.DeletePhoto(0)
	require.Error(t, err)
}
